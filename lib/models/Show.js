...
module.exports = class Show {
  ...
  static async updateById(id, attributes) {
    const currentShow = await Show.findById(id);
    const updatedAttributes = { ...currentShow, ...attributes };
    const { title, watch_app } = updatedAttributes;
    const { rows } = await pool.query(
      'UPDATE shows SET title=$1, watch_app=$2 WHERE id=$3 RETURNING *',
      [title, watch_app, id]
    );
    return new Show(rows[0]);
  }
